...
import { ArrowLeft, FileText, Download, Receipt, Eye, Share2, Mail, MessageCircle } from 'lucide-react';

const ArretePremiereAnnee = () => {
  ...
  const downloadPDF = () => {
    setDownloading(true);
    // Méthode la plus compatible pour mobile et desktop
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.setAttribute('download', 'Arrete_Premiere_Annee_SGEE.pdf');
  ...
  const shareWhatsApp = () => {
    window.open(`https://wa.me/?text=${encodeURIComponent("Voici l'arrêté officiel SGEE 2025 : " + fullUrl)}`, '_blank');
  };

  const shareEmail = () => {
    window.location.href = `mailto:?subject=Arrêté SGEE 2025&body=Consultez le document ici : ${fullUrl}`;
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <header className="bg-white border-b border-slate-200 py-4 sticky top-0 z-10 shadow-sm">
        <div className="max-w-4xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Boutons de partage ajoutés au header */}
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="text-center mb-12">
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 mb-2">Arrêté 1ère Année</h1>
          <p className="text-lg text-slate-500 font-medium">Concours Académique 2024-2025</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8">
          ...
          {hasDocument ? (
            ...
            <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center bg-slate-50">
              ...
              <p className="text-sm font-medium text-slate-500">Document en cours de validation</p>
            </div>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          <div className="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            <ul className="space-y-3">
              {['Tout Baccalauréat accepté', 'Âge minimum 14 ans requis', 'Accord parental pour les mineurs', 'Préinscription en ligne obligatoire'].map((item, i) => (
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            <ul className="space-y-3">
              {[
                ...
                { label: 'Boursiers', price: '25 000 FCFA' },
                { label: 'Étrangers', price: '150 000 FCFA' },
                { label: 'Frais Admin', price: '5 000 FCFA' }
              ].map((item, i) => (
        ...
        <div className="bg-indigo-600 p-8 rounded-2xl text-white shadow-xl shadow-indigo-100">
          <h3 className="font-bold text-lg mb-6 text-center">Calendrier du Concours</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
              <p className="text-indigo-200 text-xs font-bold uppercase mb-1">Ouverture</p>
              <p className="text-lg font-black">15 Juin</p>
            </div>
            <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm">
              <p className="text-indigo-200 text-xs font-bold uppercase mb-1">Clôture</p>
              <p className="text-lg font-black">31 Août</p>
            </div>
            <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm border border-white/20">
              <p className="text-indigo-200 text-xs font-bold uppercase mb-1">Concours</p>
              <p className="text-lg font-black">15 Sept</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <footer className="bg-slate-900 text-slate-400 py-8 mt-12">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="text-xs font-medium tracking-wide">© 2025 SGEE — RÉPUBLIQUE DU CAMEROUN</p>
        </div>
      </footer>
    </div>
  );
...
export default ArretePremiereAnnee;